...
locals {
  ...
  prefix_safe            = lower(join(var.custom-separator, var.prefix))
  suffix                 = join("-", var.suffix)
  suffix_unique          = join("-", concat(var.suffix, [local.random]))
  suffix_safe            = lower(join(var.custom-separator, var.suffix))
  suffix_unique_safe     = lower(join(var.custom-separator, concat(var.suffix, [local.random])))
  // Names based on the recommendations of
  // https://docs.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/naming-and-tagging
  az = {
    analysis_services_server = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "as", local.suffix_safe])), 0, 63)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "as", local.suffix_unique_safe])), 0, 63)
      dashes      = false
      slug        = "as"
      min_length  = 3
      max_length  = 63
    ...
    batch_account = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "ba", local.suffix_safe])), 0, 24)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "ba", local.suffix_unique_safe])), 0, 24)
      dashes      = false
      slug        = "ba"
      min_length  = 3
      max_length  = 24
    ...
    container_registry = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "acr", local.suffix_safe])), 0, 63)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "acr", local.suffix_unique_safe])), 0, 63)
      dashes      = false
      slug        = "acr"
      min_length  = 1
      max_length  = 63
    ...
    container_registry_webhook = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "crwh", local.suffix_safe])), 0, 50)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "crwh", local.suffix_unique_safe])), 0, 50)
      dashes      = false
      slug        = "crwh"
      min_length  = 1
      max_length  = 50
    ...
    data_lake_analytics_account = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "dla", local.suffix_safe])), 0, 24)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "dla", local.suffix_unique_safe])), 0, 24)
      dashes      = false
      slug        = "dla"
      min_length  = 3
      max_length  = 24
    ...
    data_lake_store = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "dls", local.suffix_safe])), 0, 24)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "dls", local.suffix_unique_safe])), 0, 24)
      dashes      = false
      slug        = "dls"
      min_length  = 3
      max_length  = 24
    ...
    fabric_capacity = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "fc", local.suffix_safe])), 0, 63)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "fc", local.suffix_unique_safe])), 0, 63)
      dashes      = false
      slug        = "fc"
      min_length  = 3
      max_length  = 63
    ...
    kusto_cluster = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "kc", local.suffix_safe])), 0, 22)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "kc", local.suffix_unique_safe])), 0, 22)
      dashes      = false
      slug        = "kc"
      min_length  = 4
      max_length  = 22
    ...
    redis_firewall_rule = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "redisfw", local.suffix_safe])), 0, 256)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "redisfw", local.suffix_unique_safe])), 0, 256)
      dashes      = false
      slug        = "redisfw"
      min_length  = 1
      max_length  = 256
    ...
    shared_image_gallery = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "sig", local.suffix_safe])), 0, 80)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "sig", local.suffix_unique_safe])), 0, 80)
      dashes      = false
      slug        = "sig"
      min_length  = 1
      max_length  = 80
    ...
    storage_account = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "st", local.suffix_safe])), 0, 24)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "st", local.suffix_unique_safe])), 0, 24)
      dashes      = false
      slug        = "st"
      min_length  = 3
      max_length  = 24
    ...
    synapse_spark_pool = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "synsp", local.suffix_safe])), 0, 32)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "synsp", local.suffix_unique_safe])), 0, 32)
      dashes      = false
      slug        = "synsp"
      min_length  = 3
      max_length  = 32
    ...
    synapse_sql_pool = {
      name        = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "syndp", local.suffix_safe])), 0, 60)
      name_unique = substr(join(lower(var.custom-separator), compact([local.prefix_safe, "syndp", local.suffix_unique_safe])), 0, 60)
      dashes      = false
      slug        = "syndp"
      min_length  = 1
      max_length  = 60
